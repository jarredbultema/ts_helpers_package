...
def aggregate_df(
    df, aggregators=None, string_columns=None, numeric_columns=None, ignore_columns=None
):
    """
    ...
    for si, col in enumerate(string_columns, 1):
        ...
        try:
            ...
            out_df[min_name] = df.groupby(aggregators).agg(
                {col: lambda x: pd.Series.value_counts(x).idxmin()}
            )
        except:
        ...
        try:
            # this causes an error with np.nan values
            out_df[max_name] = df.groupby(aggregators).agg(
                {col: lambda x: pd.Series.value_counts(x).idxmax()}
            )
        except:
    ...
    for ni, n_col in enumerate(numeric_columns, 1):
        ...
        out_df[str(n_col) + "_stdev"] = df.groupby(aggregators).agg(
            {n_col: pd.Series.std}
        )
        out_df[str(n_col) + "_unique"] = df.groupby(aggregators).agg(
            {n_col: pd.Series.nunique}
        )
    ...
    out_df["aggregated_total_count"] = df.groupby(aggregators).agg(
        {string_columns[0]: "size"}
    )
