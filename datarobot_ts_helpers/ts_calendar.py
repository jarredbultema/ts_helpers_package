...
from pandas.tseries.holiday import TH, AbstractHolidayCalendar, Holiday, USFederalHolidayCalendar
...
def create_ts_calendar(df, ts_settings, additional_events=None):
    ...
    events["Date"] = [dt.datetime.strftime(pd.to_datetime(date), "%Y-%m-%d") for date in events["Date"]]
...
def create_and_upload_ts_calendar(
    ...
    cal = dr.CalendarFile.create(
        file_path=filename, calendar_name=calendar_name, multiseries_id_columns=multiseries_id
    )
...
def plot_ts_calendar(df, ts_settings, calendar=None):
    ...
    merged = merged[
        (merged["cal_date"] >= merged[date_col]) & (merged["cal_date"] <= merged[date_col] + epsilon)
